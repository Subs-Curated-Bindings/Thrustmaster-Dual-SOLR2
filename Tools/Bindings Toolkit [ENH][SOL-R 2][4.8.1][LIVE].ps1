<#
.SYNOPSIS
    Bindings Toolkit for [Enhanced] Dual TM SOL-R. Menu-driven utility
...
.DESCRIPTION
    ...
    WORKFLOW (typical case -- right after loading the SOL-R 2 layout):
      1. Fully close Star Citizen and the RSI Launcher.
      2. Double-click "Bindings Toolkit [ENH][SOL-R 2][4.8.1][LIVE].bat".
...
.EXAMPLE
    .\Bindings Toolkit [ENH][SOL-R 2][4.8.1][LIVE].ps1
    Show the menu, prompt for channel as needed.

.EXAMPLE
    .\Bindings Toolkit [ENH][SOL-R 2][4.8.1][LIVE].ps1 -Action MFD -Channel LIVE

.EXAMPLE
    .\Bindings Toolkit [ENH][SOL-R 2][4.8.1][LIVE].ps1 -Action Invert -Channel PTU
#>
[CmdletBinding()]
param(
...
#  Refactored from the original Fix MFD Binds [ENH][SOL-R 2].ps1.
# =====================================================================
...
function Invoke-FixMfd-Channel {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        Write-Host "  Load the SOL-R 2 layout in-game first (Customization > Control Profiles)," -ForegroundColor Yellow
...
function Invoke-ClearAllBinds-Channel {
    ...
    Write-Host "    2. Customization > Control Profiles > Use this profile -- pick the SOL-R 2 layout." -ForegroundColor Cyan
...
function Test-LayoutXmlForChannel {
    # Returns @{ Present; Path; SizeKB; MTime } -- the SOL-R 2 layout file
    ...
    $found = @(Get-ChildItem -LiteralPath $mappings -Filter 'layout_ENH_SOL-R2_*_exported.xml' -ErrorAction SilentlyContinue)
    if ($found.Count -eq 0) {
        return @{ Present = $false; Reason = 'no layout_ENH_SOL-R2_*_exported.xml in mappings dir' }
    }
...
function Show-StackHealth {
    ...
    $shipped = Join-Path $ScriptRoot '..\Joystick Gremlin Profile [ENH][SOL-R 2][4.8.1][LIVE][R14].xml'
    ...
    elseif ($vj.DeviceCount -lt 2) {
        Write-Host ("  vJoy: {0} loaded, {1} device(s) configured -- SOL-R profile expects 2 (use vJoyConf to add a second)" -f $vVer, $vj.DeviceCount) -ForegroundColor Yellow
    }
    else {
...
function Invoke-ShowDiagnostic-Channel {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        elseif ($slots.Count -eq 1) {
            Write-Host ("  vJoy slots in use: {0} only -- SOL-R profile expects both js1 and js2" -f $slots[0]) -ForegroundColor Yellow
        }
        else {
