...
.EXAMPLE
    .\Fix MFD Binds [ENH][SOL-R 2][4.8.1][LIVE].ps1
    Auto-detect channels, prompt to pick.

.EXAMPLE
    .\Fix MFD Binds [ENH][SOL-R 2][4.8.1][LIVE].ps1 -Channel PTU

.EXAMPLE
    .\Fix MFD Binds [ENH][SOL-R 2][4.8.1][LIVE].ps1 -InstallRoot 'D:\Games\StarCitizen' -Channel PTU
#>
[CmdletBinding()]
param(
...
if (-not (Test-Path -LiteralPath $InstallRoot)) {
    ...
    if (-not (Test-Path -LiteralPath $entered)) {
        ...
        Write-Host "  .\Fix MFD Binds [ENH][SOL-R 2][4.8.1][LIVE].ps1 -InstallRoot 'X:\path\to\StarCitizen'" -ForegroundColor Yellow
        exit 1
    }
    $InstallRoot = $entered
    Write-Host ""
