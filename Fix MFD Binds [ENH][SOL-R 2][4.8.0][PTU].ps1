<#
.SYNOPSIS
    Re-applies the [Enhanced] Dual TM SOL-R 2 MFD binds to Star Citizen's
    live actionmaps.xml.
...
.DESCRIPTION
    ...
    WORKFLOW:
      1. Load the [ENH][SOL-R 2] layout in-game once (Customization > Control
...
.EXAMPLE
    .\Fix MFD Binds [ENH][SOL-R 2][4.8.0][PTU].ps1
    Auto-detect channels, prompt to pick.

.EXAMPLE
    .\Fix MFD Binds [ENH][SOL-R 2][4.8.0][PTU].ps1 -Channel PTU

.EXAMPLE
    .\Fix MFD Binds [ENH][SOL-R 2][4.8.0][PTU].ps1 -InstallRoot 'D:\Games\StarCitizen' -Channel PTU
#>
[CmdletBinding()]
param(
...
# ===================================================================
#  STICK-SPECIFIC BIND DATA -- TM SOL-R 2 MFD layout
#  Do not edit unless you are customizing your own SOL-R 2 MFD bindings.
# ===================================================================
$Binds = @(
...
$StickName = '[Enhanced] Dual TM SOL-R 2'
...
if (-not (Test-Path -LiteralPath $InstallRoot)) {
    ...
    Write-Host "  .\Fix MFD Binds [ENH][SOL-R 2][4.8.0][PTU].ps1 -InstallRoot 'X:\path\to\StarCitizen'" -ForegroundColor Yellow
    exit 1
}
...
foreach ($ch in $targets) {
    ...
    if (-not $mfdMatch.Success) {
        ...
        Write-Host "  Load the [ENH][SOL-R 2] layout in-game first (Customization > Control Profiles)," -ForegroundColor Yellow
